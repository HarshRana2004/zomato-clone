...
import { getSmartSuggestion } from "../utils/predictOrder";
...
export default function Orders() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      {suggestion && (
        <div className="bg-yellow-100 border border-yellow-300 rounded-md p-4 mb-4 animate-pulse">
          <p className="text-sm text-gray-700 mb-2">
            🔁 Looks like you usually order <strong>{suggestion.item}</strong> on <strong>{suggestion.day}</strong>.
          </p>
          <button
            className="bg-red-500 text-white px-4 py-2 rounded-full text-sm"
            onClick={() => {
              const pastOrders = JSON.parse(localStorage.getItem("foodyOrders")) || [];
              const match = pastOrders.reverse().find((o) =>
                o.items.some((i) => i.name === suggestion.item)
              );
              if (match) {
                localStorage.setItem("foodyCart", JSON.stringify(match.items));
                localStorage.setItem("foodyCartRestaurant", match.restaurantId);
                navigate("/cart");
              }
            }}
          >
